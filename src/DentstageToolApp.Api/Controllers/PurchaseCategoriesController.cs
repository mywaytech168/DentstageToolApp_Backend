...
public class PurchaseCategoriesController : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyCollection<PurchaseCategoryDto>>> GetCategoriesAsync(CancellationToken cancellationToken)
    {
        try
        {
            var response = await _purchaseService.GetCategoriesAsync(cancellationToken);
            return Ok(response);
        }
