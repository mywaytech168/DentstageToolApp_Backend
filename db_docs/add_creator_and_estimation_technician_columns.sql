...
ALTER TABLE Quatations
    ADD COLUMN TechnicianUID VARCHAR(100) NULL AFTER UserName,
    ADD COLUMN CreatorTechnicianUID VARCHAR(100) NULL AFTER TechnicianUID;

-- 以既有估價資料回填估價技師欄位，維持舊資料可用性。
UPDATE Quatations
SET TechnicianUID = UserUID
WHERE TechnicianUID IS NULL AND UserUID IS NOT NULL;
